...
app.MapPost(
        ...
        ) =>
        {
            ...
            try
            {
                ...
                var oldTokens = await db
                    .RefreshTokens.Where(rt => rt.UserId == user.Id && rt.IsRevoked == false)
                    .ToListAsync();
                ...
                Log.Information(
                    "Login successful for user: {Username} (ID: {UserId}) from IP: {IpAddress}",
                    user.Username,
                    user.Id,
                    ipAddress
                );
                ...
                return Results.Ok(
                    new AuthResponse(accessToken, refreshToken, user.Username, user.Id)
                );
            }
            catch (Exception ex)
            {
                Log.Error(
                    ex,
                    "Login error for username: {Username} from IP: {IpAddress}",
                    request.Username,
                    ipAddress
                );
...
app.MapPost(
        ...
        ) =>
        {
            ...
            Log.Information(
                "Register attempt for username: {Username} from IP: {IpAddress}, InviteToken: {InviteToken}",
                request.Username,
                ipAddress,
                request.InviteToken ?? "None"
            );
            ...
            try
            {
                ...
                {
                    Log.Warning(
                        "Register failed - Username already exists: {Username} from IP: {IpAddress}",
                        request.Username,
                        ipAddress
                    );
                ...
                if (string.IsNullOrEmpty(request.InviteToken))
                {
                    ...
                    Log.Information(
                        "New couple created for user: {Username} (ID: {UserId}), Couple ID: {CoupleId}, InviteToken: {InviteToken}",
                        user.Username,
                        user.Id,
                        couple.Id,
                        inviteToken
                ...
                else
                {
                    ...
                    var couple = await db.Couples.FirstOrDefaultAsync(c =>
                        c.InviteToken == request.InviteToken && c.IsActive
                    );
                    if (couple == null)
                    {
                        Log.Warning(
                            "Register failed - Invalid invite token: {InviteToken} for username: {Username} from IP: {IpAddress}",
                            request.InviteToken,
                            request.Username,
                            ipAddress
                        );
                    ...
                    {
                        Log.Warning(
                            "Register failed - Couple is full: {InviteToken} for username: {Username} from IP: {IpAddress}",
                            request.InviteToken,
                            request.Username,
                            ipAddress
                        );
                    ...
                    Log.Information(
                        "User joined existing couple: {Username} (ID: {UserId}), Couple ID: {CoupleId}",
                        user.Username,
                        user.Id,
                        couple.Id
                    );
                ...
                Log.Information(
                    "Register successful for user: {Username} (ID: {UserId}) from IP: {IpAddress}",
                    user.Username,
                    user.Id,
                    ipAddress
                );
                ...
                return Results.Ok(
                    new AuthResponse(accessToken, refreshToken, user.Username, user.Id, inviteToken)
                );
            }
            catch (Exception ex)
            {
                Log.Error(
                    ex,
                    "Register error for username: {Username} from IP: {IpAddress}",
                    request.Username,
                    ipAddress
                );
                return Results.StatusCode(500);
...
app.MapPost(
        "/auth/refresh",
        async (RefreshTokenRequest request, AppDbContext db, JwtService jwtService) =>
        {
            // Refresh token'ı veritabanından bul
            var refreshTokenEntity = await db.RefreshTokens.FirstOrDefaultAsync(rt =>
                rt.Token == request.RefreshToken && rt.IsRevoked == false
            );
...
app.MapPost(
        "/auth/logout",
        async (RefreshTokenRequest request, AppDbContext db) =>
        {
...
app.MapGet(
        ...
        {
            var userId = GetCurrentUserId(user);
            var todoLists = await db
                .TodoLists.Where(tl => tl.OwnerId == userId)
...
app.MapGet(
        ...
        async (ClaimsPrincipal user, AppDbContext db) =>
        {
            ...
            var currentUser = await db
                .Users.Include(u => u.Couple)
                .FirstOrDefaultAsync(u => u.Id == userId);

            if (currentUser?.CoupleId == null)
                return Results.BadRequest("Henüz bir couple'a ait değilsiniz.");

            var partnerTodoLists = await db
                .TodoLists.Where(tl =>
                    tl.Owner.CoupleId == currentUser.CoupleId && tl.OwnerId != userId
                )
                .OrderByDescending(tl => tl.CreatedAt)
                .Select(tl => new TodoListResponse(
                    tl.Id,
...
app.MapPost(
        "/todolists",
        async (CreateTodoListRequest request, ClaimsPrincipal user, AppDbContext db) =>
        {
            var userId = GetCurrentUserId(user);
...
app.MapPut(
        "/todolists/{id}",
        async (int id, UpdateTodoListRequest request, ClaimsPrincipal user, AppDbContext db) =>
        {
            var userId = GetCurrentUserId(user);

            // TodoList'in kullanıcıya ait olduğunu veya partner'ın olduğunu kontrol et
            var todoList = await db
                .TodoLists.Include(tl => tl.Owner)
            ...
            if (todoList.OwnerId != userId && !todoList.IsShared)
                return Results.BadRequest(
                    "Bu todo list'i düzenleme yetkiniz yok. Sadece okuma yapabilirsiniz."
                );
...
app.MapDelete(
        "/todolists/{id}",
        async (int id, ClaimsPrincipal user, AppDbContext db) =>
        {
            var userId = GetCurrentUserId(user);
            ...
            if (todoList.OwnerId != userId && !todoList.IsShared)
                return Results.BadRequest(
                    "Bu todo list'i silme yetkiniz yok. Sadece okuma yapabilirsiniz."
                );
...
app.MapGet(
        ...
        async (int todoListId, ClaimsPrincipal user, AppDbContext db) =>
        {
            var userId = GetCurrentUserId(user);
            ...
            var todoItems = await db
                .TodoItems.Where(ti => ti.TodoListId == todoListId)
...
app.MapPost(
        ...
        ) =>
        {
            ...
            if (todoList.OwnerId != userId && !todoList.IsShared)
                return Results.BadRequest(
                    "Bu todo list'e item ekleme yetkiniz yok. Sadece okuma yapabilirsiniz."
                );
            ...
            CreateActivity(
                ...
                todoListId,
                todoList.Title,
                $"\"{todoItem.Title}\" öğesi \"{todoList.Title}\" listesine eklendi"
            );
            await db.SaveChangesAsync();
...
app.MapPut(
        ...
        ) =>
        {
            ...
            if (todoList.OwnerId != userId && !todoList.IsShared)
                return Results.BadRequest(
                    "Bu todo list'in itemlarını güncelleme yetkiniz yok. Sadece okuma yapabilirsiniz."
                );

            var todoItem = await db.TodoItems.FirstOrDefaultAsync(ti =>
                ti.Id == itemId && ti.TodoListId == todoListId
            );
            ...
            if (oldStatus != newStatus)
            {
                if (newStatus == TodoStatus.Done)
                {
                    CreateActivity(
                        ...
                        todoListId,
                        todoList.Title,
                        $"\"{todoItem.Title}\" öğesi \"{todoList.Title}\" listesinde tamamlandı"
                    );
                }
                else if (oldStatus == TodoStatus.Done && newStatus == TodoStatus.Pending)
                {
                    CreateActivity(
                        ...
                        todoListId,
                        todoList.Title,
                        $"\"{todoItem.Title}\" öğesi \"{todoList.Title}\" listesinde yeniden açıldı"
                    );
                }
            }
            else
            {
                ...
                CreateActivity(
                    ...
                    todoListId,
                    todoList.Title,
                    $"\"{todoItem.Title}\" öğesi \"{todoList.Title}\" listesinde güncellendi"
                );
            }
...
app.MapDelete(
        "/todolists/{todoListId}/items/{itemId}",
        async (int todoListId, int itemId, ClaimsPrincipal user, AppDbContext db) =>
        {
            var userId = GetCurrentUserId(user);
            ...
            if (todoList.OwnerId != userId && !todoList.IsShared)
                return Results.BadRequest(
                    "Bu todo list'in itemlarını silme yetkiniz yok. Sadece okuma yapabilirsiniz."
                );
            ...
            CreateActivity(
                ...
                todoListId,
                todoList.Title,
                $"\"{todoItemTitle}\" öğesi \"{todoList.Title}\" listesinden silindi"
            );
            await db.SaveChangesAsync();
...
// TodoList search - kendi ve partner'ın todo listelerinde arama
app.MapGet(
        "/todolists/search",
        async (string q, ClaimsPrincipal user, AppDbContext db) =>
        {
            ...
            var coupleUserIds = await db
                .Users.Where(u => u.CoupleId == currentUser.CoupleId)
                .Select(u => u.Id)
                .ToListAsync();

            // Title veya description içinde arama yap
            var results = await db
                .TodoLists.Where(tl =>
                    coupleUserIds.Contains(tl.OwnerId)
                    && (
                        tl.Title.ToLower().Contains(searchTerm)
                        || tl.Description.ToLower().Contains(searchTerm)
                    )
                )
            ...
            return Results.Ok(new TodoListSearchResponse(results, results.Count));
        }
    )
    .WithName("SearchTodoLists")
    .RequireAuthorization();

// TodoItem search - tüm todo item'larda arama (couple bazlı)
app.MapGet(
        "/todoitems/search",
        async (string q, ClaimsPrincipal user, AppDbContext db) =>
        {
            ...
            // Title veya description içinde arama yap
            var results = await db
                .TodoItems.Include(ti => ti.TodoList)
                .Where(ti =>
                    coupleUserIds.Contains(ti.TodoList.OwnerId)
                    && (
                        ti.Title.ToLower().Contains(searchTerm)
                        || (ti.Description != null && ti.Description.ToLower().Contains(searchTerm))
                    )
                )
                .OrderByDescending(ti => ti.UpdatedAt)
                .Select(ti => new TodoItemWithListResponse(
                    ti.Id,
                    ti.Title,
                    ti.Description,
                    ti.Status,
                    ...
                    ti.TodoListId,
                    ti.TodoList.Title,
                    ti.TodoList.ColorCode
                ))
                .ToListAsync();

            return Results.Ok(new TodoItemSearchResponse(results, results.Count));
        }
    )
    .WithName("SearchTodoItems")
    .RequireAuthorization();

// TodoItem search - belirli bir liste içinde arama
app.MapGet(
        "/todolists/{todoListId}/items/search",
        async (int todoListId, string q, ClaimsPrincipal user, AppDbContext db) =>
        {
            if (string.IsNullOrWhiteSpace(q))
                return Results.BadRequest("Arama sorgusu boş olamaz.");
    ...
    .WithName("SearchTodoItemsInList")
    .RequireAuthorization();

// ========== REORDER ENDPOINT ==========

// TodoItem reorder - sıralama değişikliği (drag & drop desteği)
app.MapPut(
        "/todolists/{todoListId}/items/reorder",
        async (
            int todoListId,
            ReorderTodoItemsRequest request,
            ClaimsPrincipal user,
            AppDbContext db
        ) =>
        {
            ...
            if (todoList == null)
                return Results.NotFound("Todo list bulunamadı.");
            ...
            // Yetki kontrolü: Sadece owner veya shared list ise reorder yapabilir
            if (todoList.OwnerId != userId && !todoList.IsShared)
                return Results.BadRequest(
                    "Bu todo list'in itemlarını sıralama yetkiniz yok. Sadece okuma yapabilirsiniz."
                );

            if (request.Items == null || !request.Items.Any())
                return Results.BadRequest("Sıralanacak item listesi boş olamaz.");
            ...
            // Request'teki item'ların bu listeye ait olduğunu kontrol et
            var requestItemIds = request.Items.Select(i => i.ItemId).ToHashSet();
            var existingItemIds = todoItems.Select(ti => ti.Id).ToHashSet();

            if (!requestItemIds.All(id => existingItemIds.Contains(id)))
                return Results.BadRequest("Bazı item'lar bu listeye ait değil.");

            // Sıralamayı güncelle
            foreach (var reorderItem in request.Items)
            {
                var todoItem = todoItems.FirstOrDefault(ti => ti.Id == reorderItem.ItemId);
                if (todoItem != null)
                {
                    todoItem.Order = reorderItem.NewOrder;
                    todoItem.UpdatedAt = DateTime.UtcNow;
                }
            }

            // TodoList'in UpdatedAt'ini güncelle
            todoList.UpdatedAt = DateTime.UtcNow;

            await db.SaveChangesAsync();

            // Güncellenmiş item listesini döndür
            var updatedItems = await db
                .TodoItems.Where(ti => ti.TodoListId == todoListId)
                .OrderBy(ti => ti.Order)
                .Select(ti => new TodoItemResponse(
                    ti.Id,
...
app.MapPut(
        "/users/profile",
        async (UpdateProfileRequest request, ClaimsPrincipal user, AppDbContext db) =>
        {
            var userId = GetCurrentUserId(user);
...
app.MapGet(
        ...
        async (ClaimsPrincipal user, AppDbContext db) =>
        {
            ...
            var partner = await db.Users.FirstOrDefaultAsync(u =>
                u.CoupleId == currentUser.CoupleId && u.Id != userId
            );
            ...
            var todoLists = await db
                .TodoLists.Where(tl => tl.OwnerId == partner.Id)
                .Include(tl => tl.TodoItems)
                .OrderByDescending(tl => tl.CreatedAt)
                .ToListAsync();

            // Response oluştur
            var todoListsWithItems = todoLists
                .Select(tl => new TodoListWithItemsResponse(
                    tl.Id,
                    tl.Title,
                    tl.Description,
                    tl.OwnerId,
                    tl.CreatedAt,
                    tl.UpdatedAt,
                    tl.TodoItems.OrderByDescending(ti => ti.CreatedAt)
                        .Select(ti => new TodoItemResponse(
                            ti.Id,
                            ti.Title,
                            ti.Description,
...
app.MapGet(
        ...
        async (ClaimsPrincipal user, AppDbContext db) =>
        {
            ...
            var allTodoItems = await db
                .TodoItems.Where(ti =>
                    db.TodoLists.Any(tl =>
                        tl.Id == ti.TodoListId && coupleUserIds.Contains(tl.OwnerId)
                    )
                )
                .ToListAsync();

            // Kullanıcının kendi task'leri
            var myTodoItems = await db
                .TodoItems.Where(ti =>
                    db.TodoLists.Any(tl => tl.Id == ti.TodoListId && tl.OwnerId == userId)
                )
                .ToListAsync();
            ...
            var completedToday = allTodoItems.Count(ti =>
                ti.Status == TodoStatus.Done && ti.UpdatedAt >= today && ti.UpdatedAt < tomorrow
            );
...
app.MapGet(
        "/activities/recent",
        async (ClaimsPrincipal user, AppDbContext db, int limit = 10) =>
        {
            var userId = GetCurrentUserId(user);
            var currentUser = await db
                .Users.Include(u => u.Couple)
            ...
            var activities = await db
                .Activities.Where(a => coupleUserIds.Contains(a.UserId))
                .Include(a => a.User)
